...
import { Settings, ShoppingCart, BarChart3, Plus, Trash2, Store, Package, Save, Upload, Download, Zap, DollarSign, Target, FileSpreadsheet, Info, ArrowUpDown, Edit3, Lightbulb, MoreVertical, Sun, Moon, Sliders, CalendarDays, Compass, Activity, Wand2, Database, RefreshCw, Layers, ClipboardList } from 'lucide-react';
...
export default function App() {
  ...
  const themes = {
    dark: {
      ...
      border: "border-zinc-800", input: "bg-zinc-950 border-zinc-700 text-white placeholder-gray-600 focus:ring-purple-500",
      inputYellow: "bg-zinc-950 border-zinc-700 text-yellow-400 font-bold focus:ring-yellow-500 placeholder-gray-600",
    ...
    light: {
      ...
      border: "border-gray-200", input: "bg-white border-gray-300 text-gray-900 placeholder-gray-400 focus:ring-blue-500",
      inputYellow: "bg-white border-gray-300 text-indigo-700 font-bold focus:ring-indigo-500 placeholder-gray-400",
  ...
  // --- LÓGICA DE CLUSTERIZACIÓN DINÁMICA Y EXPONENCIAL ---
  const recalculateClusters = (rawData, weights, currentClusters, strategy) => {
    ...
    Object.keys(dataByGoa).forEach(goaName => {
      ...
      // Ordenar tiendas de mayor a menor score
      ...
      storesInGoa.forEach((item, index) => {
        const percentile = index / total; // de 0.0 (la mejor) a 1.0 (la peor)
        let clusterIndex = numClust - 1; // Por defecto el clúster más bajo
        
        if (strategy === 'piramide') {
          if (numClust === 6) {
            // Pirámide Retail Exacta: 5% AA | 15% A | 25% B | 30% C | 15% D | 10% E
            ...
            // Fórmula cuadrática para hacer la punta pequeña y la base grande
            let assigned = false;
          ...
          // Basado 100% en la distancia al score máximo
  ...
  // --- SINCRONIZACIÓN CON GO FORECASTING ---
               ...
               <>
                    ...
                    <table className="w-full text-left text-sm border-collapse">
                      <thead>
                        <tr className={`text-xs uppercase border-b tracking-wider ${t.tableHead}`}>
                          ...
                          <th className={`p-4 text-right border-l font-bold ${t.border} ${t.textMain}`}>$ Invertido</th>
                      ...
                      <tbody className={`divide-y ${t.border}`}>
                        {reportData.goaMetrics.map(g => (
                          <tr key={g.id} className={`transition ${t.tableRow}`}>
                            ...
                            <td className={`p-4 text-right font-bold ${g.historyDiff >= 0 ? t.textAccent1 : t.dangerText}`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className={`rounded-xl border shadow-lg p-6 max-h-[600px] overflow-y-auto custom-scrollbar ${t.card}`}>
                    <h2 className={`text-lg font-bold mb-4 flex items-center ${t.textMain}`}><Store className={`mr-3 ${t.textAccent1}`}/> Assortment por Tienda</h2>
                    <table className="w-full text-left text-sm border-collapse">
                      ...
                      <tbody className={`divide-y ${t.border}`}>
                        {reportData.storeSummary.length === 0 && (
                          <tr><td colSpan="4" className={`p-8 text-center text-xs italic ${t.textMuted}`}>Aún no hay compras registradas en esta vista.</td></tr>
                        )}
                              ...
                              <td className="p-3 text-center">
                                <div className="flex flex-wrap gap-1 justify-center">
                                  {Object.entries(s.clusters || {}).map(([goa, c]) => (
                                    <span key={`store-${s.id}-${goa}`} className={`text-[9px] px-1.5 py-0.5 rounded font-bold border ${c === activeClusters[0] ? t.badgeAA : c === activeClusters[1] ? t.badgeA : t.badgeOther}`} title={goa}>{c}</span>
                                  ))}
                                </div>
                              </td>
                  ...
                  <div className={`rounded-xl border shadow-lg p-6 max-h-[600px] overflow-y-auto custom-scrollbar ${t.card}`}>
                    <h2 className={`text-lg font-bold mb-4 flex items-center ${t.textMain}`}>
                      <FileSpreadsheet className="mr-3 text-green-500"/> Matriz de Distribución
                    </h2>
                    
                    {Object.keys(reportData.matrixByGoa || {}).length === 0 ? (
                      <p className={`text-sm italic p-4 text-center ${t.textMuted}`}>No hay modelos suficientes para graficar.</p>
                    ) : null}
                      ...
                      return (
                        <div key={`matriz-${goaName}`} className={`mb-8 border rounded-xl overflow-hidden shadow-sm ${t.border}`}>
                          <div className={`border-b p-3 font-black text-sm uppercase tracking-widest text-center flex justify-between items-center px-5 ${t.cardInner} ${t.textMain}`}>
                            ...
                            <span className="w-1/3 text-right"><span className={`text-[9px] px-2 py-1 rounded ${t.badgeOther}`}>Asignación</span></span>
                          </div>
                          <table className="w-full text-center text-sm border-collapse">
                            <thead>
