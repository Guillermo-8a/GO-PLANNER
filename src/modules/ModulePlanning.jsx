    ...
    <div className="flex items-end gap-0.5" style={{ height, minWidth: 80 }}>
      {valores.map((v, i) => (
        <div key={i}
          ...
          title={`M${i+1}: ${Math.round(v).toLocaleString()}`}
        />
      ))}
    </div>
  );
...
export default function Forecast() {
  ...
  // Bloque maestro Excel (Tab 3) - 4 inputs primarios, lo demás se deriva
  ...
  // Tab 5 - Plan x Tienda
  const [t5Centro, setT5Centro]                 = useState('');
  ...
  // Sugerencia inteligente: si centro >= GOA prom → +5%; si centro < GOA → híbrido 50/50
  const rotSugeridaInteligente = useMemo(() => {
    ...
    Object.entries(rotacionHistPromedio).forEach(([k, rotCentro]) => {
      ...
      out[k] = rotCentro >= rotGoa ? rotCentro * 1.05 : rotCentro * 0.5 + rotGoa * 0.5;
    });
    return out;
  }, [rotacionHistPromedio]);

  // ══════════════════════════════════════════════════════════════════════
  // TAB 4 — Plan distribuido (datos agrupados)
  // ══════════════════════════════════════════════════════════════════════
  const t4DataAgrupada = useMemo(() => {
    ...
    const groupBy = t4Agrupacion === 'centro' ? 'centro' : 'goa';
    const map = {};
    cruces.forEach(c => {
      const id = c[groupBy];
      const label = groupBy === 'centro' ? c.nombre : c.goa;
      if (!map[id]) map[id] = {
        id, nombre: label, hijos: [],
        meses: Array(12).fill(0), total: 0
      };
      map[id].hijos.push(c);
      c.meses.forEach((m, i) => map[id].meses[i] += m.valor);
  ...
  const rebalancearTab4 = (grupoId) => {
    ...
    if (!grupo) return;
    const targetBase = grupo.hijos.reduce(
      (s,h) => s + h.meses.reduce((a,m) => a + m.sugerido, 0), 0
    );
    const diff = targetBase - grupo.total;
    ...
    const sumaCand = candidatos.reduce((s,c) => s + c.valor, 0);
    if (sumaCand === 0) { alert('Todas las celdas están bloqueadas.'); return; }
    startTransition(() => {
      setT4Overrides(prev => {
        const next = { ...prev };
        candidatos.forEach(c => {
          const prop = c.valor / sumaCand;
          next[c.k] = Math.max(0, c.valor + diff * prop);
        });
        return next;
      });
    });
  ...
  // ══════════════════════════════════════════════════════════════════════
  // TAB 5 — Plan x Tienda: 8 ratios + cascada + rebalanceo de compras negativas
  // ══════════════════════════════════════════════════════════════════════
  const planCrucesMensual = useMemo(() => {
    ...
    return planCruceCompleto.map(r => {
      ...
      const rotAnual = r.rotAplicada || 1.5;

      // Mg% objetivo (override centro > GOA > histórico)
      const mgOverrideCxG = mgObjetivoCentro[`${r.centro}|${r.goa}`];
      const mgObjetivo = mgOverrideCxG != null ? mgOverrideCxG : r.mgHist;

      // Apertura
      const tipoCentro = centros[r.centro]?.tipo || 'NORMAL';
      const mesApertura = centros[r.centro]?.mesApertura || aperturas[r.centro]?.mesApertura;
      const esApertura = (tipoCentro === 'APERTURA' || tipoCentro === 'NUEVA') && mesApertura;
      const mesPreApertura = esApertura && mesApertura > 1 ? mesApertura - 1 : null;

      // Inv Inicial Ene del cruce (proporcional)
      const totMaster = otbMasterCalc.filas.reduce((s,f) => s+f.venta, 0);
      const cruceParticipacion = ventaTotal > 0 && totMaster > 0 ? ventaTotal / totMaster : 0;
      const invIniCruceEne = totMaster > 0
        ? otbMasterCalc.filas[0].invInicial * cruceParticipacion
        : (ventaMensualAjustada[0] / rotAnual);

      // INVENTARIO POR ROTACIÓN + picos
      const ventaPromMensual = ventaTotal / 12;
      const picoUmbral = ventaPromMensual * 1.2;
      const invObjetivoPorMes = ventaMensualAjustada.map(v => {
        const invProm = rotAnual > 0 ? v / rotAnual : 0;
        const extraPico = v > picoUmbral ? (v - picoUmbral) : 0;
        return invProm + extraPico;
      });

      // 1er pase: compras brutas
      let invInicial = invIniCruceEne;
      const comprasBrutas = [];
      for (let i = 0; i < 12; i++) {
      ...
      const filasMensual = [];
      for (let i = 0; i < 12; i++) {
        const venta = ventaMensualAjustada[i];
        ...
        let bonif = 0;
        if (esApertura && mesPreApertura && (i + 1) === mesPreApertura) {
          const compraApertura = comprasAjustadas[mesApertura - 1] || 0;
          bonif = compraApertura * pctBonifApertura;
        }

        // Cascada Mg objetivo: Utilidad = Venta × Mg% → Costo derivado
        const utilidad = venta * mgObjetivo + bonif;
        const costo = venta - mkd - msi + bonif - utilidad;
        const mgReal = venta > 0 ? utilidad / venta : 0;

        filasMensual.push({
          mes: i + 1,
          invInicial, venta, mkd, msi, costo, compra, invFinal,
          utilidad, mgPct: mgReal, bonif, rotacion: rotAnual,
          invObjetivo: invObjetivoPorMes[i],
          esPico: venta > picoUmbral,
          compraRebalanceada: comprasBrutas[i] < 0,
          aplica: venta > 0,
        });
        invInicial = invFinal;
      }
      ...
      const tot = filasMensual.reduce((acc, f) => ({
        ...
        compra: acc.compra + f.compra,
        utilidad: acc.utilidad + f.utilidad,
        bonif: acc.bonif + f.bonif,
      }), { venta: 0, mkd: 0, msi: 0, costo: 0, compra: 0, utilidad: 0, bonif: 0 });

      const mgRealTotal = tot.venta > 0 ? tot.utilidad / tot.venta : 0;

      // Alertas comerciales
      const mkdRatio = tot.venta > 0 ? tot.mkd / tot.venta : 0;
      const alertas = [];
      if (mkdRatio > 0.15 && mgRealTotal > 0.50) {
        alertas.push({ tipo: 'warning', msg: 'Mg alto con markdown alto: revisa Mg objetivo' });
      }
      if (mkdRatio > 0.20 && rotAnual < 2) {
        alertas.push({ tipo: 'warning', msg: 'Markdown alto sin rotación rápida — riesgo de exceso' });
      }
      if (mgObjetivo < 0.10) {
        alertas.push({ tipo: 'danger', msg: 'Mg objetivo muy bajo (<10%)' });
      }

      return {
        ...r,
        filasMensual, totales: tot, mgRealTotal,
        tipoCentro, mesApertura, mesPreApertura, esApertura,
        mgObjetivo, mgOverrideCxG: mgOverrideCxG != null,
        alertas,
      };
    }).filter(Boolean);
  ...
  const planPorCentro = useMemo(() => {
    const map = {};
    planCrucesMensual.forEach(cruce => {
      if (!map[cruce.centro]) {
        map[cruce.centro] = {
          centro: cruce.centro, nombre: cruce.nombre,
          cruces: [],
          filasMensual: Array(12).fill(null).map((_, i) => ({
            mes: i+1, invInicial: 0, venta: 0, mkd: 0, msi: 0, costo: 0,
            compra: 0, invFinal: 0, utilidad: 0, bonif: 0,
          })),
          totales: { venta: 0, mkd: 0, msi: 0, costo: 0, compra: 0, utilidad: 0, bonif: 0 },
        };
      }
      map[cruce.centro].cruces.push(cruce);
      cruce.filasMensual.forEach((f, i) => {
        ['invInicial','venta','mkd','msi','costo','compra','invFinal','utilidad','bonif'].forEach(k => {
          map[cruce.centro].filasMensual[i][k] += f[k];
        });
      });
      Object.keys(cruce.totales).forEach(k => {
        map[cruce.centro].totales[k] += cruce.totales[k];
      });
    });
    Object.values(map).forEach(c => {
      c.mgRealTotal = c.totales.venta > 0 ? c.totales.utilidad / c.totales.venta : 0;
      c.filasMensual.forEach(f => {
        f.mgPct = f.venta > 0 ? f.utilidad / f.venta : 0;
      });
    });
  ...
  return (
    <div className={`min-h-screen p-4 md:p-6 ${t.appBg} animate-fade-in-up`}>
      ...
      <div className={`rounded-2xl border overflow-hidden ${t.card}`}>
              ...
              <>
                ...
                <div className={`p-4 rounded-xl border ${t.cardInner}`}>
                  <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                    ...
                    <span className={`text-[10px] px-2 py-1 rounded ${t.badgeGray}`}>
                      Amarillo = input · Morado = derivado
                    </span>
                  </div>
                        ...
                        <tr>
                          <td className={`p-2 font-bold sticky left-0 bg-inherit ${t.textMain}`}>
                            ...
                            <select
                              ...
                              <option value="total">Total→distrib</option>
                            </select>
                          </td>
                          {otbMasterCalc.filas.map((f, i) => (
                            <td key={i} className="p-1 text-right">
                  ...
                  <p className={`text-[10px] mt-3 ${t.textMuted}`}>
                    💡 Los inputs <span className={t.textYellow}>amarillos</span> son los 4 que tú das (Vta, Inv Inicial Ene, Mg%, Rot). El sistema deriva el resto.
                  </p>
                </div>
        ...
        {/* ══════════ TAB 4: MATRIZ GOA × CENTRO (Plan distribuido) ══════════ */}
        {activeTab === 4 && (
          <div className="p-5 space-y-5">
            {!historico.length && (
              <div className={`p-8 rounded-xl border flex flex-col items-center justify-center text-center ${t.cardInner}`}>
            ...
            {historico.length > 0 && t4DataAgrupada.length > 0 && (
              <>
                ...
                <div className={`p-4 rounded-xl border ${t.cardInner} flex items-center justify-between flex-wrap gap-3`}>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <input placeholder="Filtrar..." value={t4Filtro} onChange={e => setT4Filtro(e.target.value)}
                      className={`text-xs px-3 py-1.5 rounded border ${t.input} focus:outline-none focus:ring-1`} />
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] ${t.textMuted}`}>{t4Locks.size} bloqueadas</span>
                    <button onClick={() => setT4Expandidas(new Set())}
                      className={`text-xs px-3 py-1.5 rounded ${t.btnGhost}`}>Colapsar todo</button>
                    <button onClick={() => setT4Expandidas(new Set(t4DataAgrupada.map(g => g.id)))}
                      className={`text-xs px-3 py-1.5 rounded ${t.btnGhost}`}>Expandir todo</button>
                  </div>
                </div>
                ...
                <div className={`rounded-xl border ${t.cardInner} overflow-hidden`}>
                  <div className="overflow-x-auto custom-scrollbar max-h-[70vh]">
                    <table className="w-full text-left min-w-max text-xs">
                      ...
                      <tbody>
                        {t4DataAgrupada
                          .filter(g => !t4Filtro || (g.nombre || g.id).toLowerCase().includes(t4Filtro.toLowerCase()))
                          .map(grupo => {
                              ...
                              <React.Fragment key={grupo.id}>
                                ...
                                <tr className={`font-bold ${isDark ? 'bg-violet-900/10 hover:bg-violet-900/20' : 'bg-violet-50 hover:bg-violet-100'} cursor-pointer`}
                                  ...
                                  <td className={`p-2 sticky left-0 bg-inherit ${t.textMain}`}>
                                    ...
                                    {grupo.nombre || grupo.id} <span className={`text-[9px] ${t.textMuted}`}>({grupo.hijos.length})</span>
                                  </td>
                                  {grupo.meses.map((v, i) => (
                                  ...
                                  <td className="p-2 text-center">
                                    <button onClick={(e) => { e.stopPropagation(); rebalancearTab4(grupo.id); }}
                                      ...
                                      title="Rebalancear celdas no bloqueadas">⚖️</button>
                                  </td>
                                </tr>
                                {/* FILAS HIJAS */}
                                {expandido && grupo.hijos.map(hijo => (
                                  <tr key={`${hijo.centro}|${hijo.goa}`} className={`text-xs ${isDark ? 'hover:bg-zinc-800/30' : 'hover:bg-violet-50/30'}`}>
                                    <td className={`p-2 pl-8 sticky left-0 bg-inherit ${t.textMuted}`}>
                                      {t4Agrupacion === 'centro'
                                        ? <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${t.badgePurple}`}>{hijo.goa}</span>
                                          ...
                                          <div className="flex items-center justify-end gap-0.5">
                                            <NumberInputDeferred
                                              ...
                                              onCommit={(parsed) => {
                                                if (parsed === null) {
                                                  ...
                                                  setT4Overrides(prev => ({ ...prev, [k]: parsed }));
                                            ...
                                            {m.isLocked && (
                                              <button onClick={() => {
                                                setT4Overrides(prev => { const n = {...prev}; delete n[k]; return n; });
                                                setT4Locks(prev => { const n = new Set(prev); n.delete(k); return n; });
                                              }} className="text-[10px]" title="Liberar">🔒</button>
                                            )}
                                          </div>
                                        </td>
                                      );
                ...
                <p className={`text-[10px] ${t.textMuted}`}>
                  💡 Click en un valor → se marca 🔒 (intocable). Click en el 🔒 → libera. Botón ⚖️ rebalancea la diferencia solo entre celdas sin candado.
                </p>
              </>
            )}
          </div>
        ...
        {activeTab === 5 && (
          <div className="p-5 space-y-5">
            {!historico.length && (
              <div className={`p-8 rounded-xl border flex flex-col items-center justify-center text-center ${t.cardInner}`}>
                <p className={`text-sm font-bold ${t.textMain}`}>Carga el histórico (Tab 1) y configura drivers (Tab 3)</p>
              </div>
            )}
            ...
            {historico.length > 0 && planCrucesMensual.length > 0 && (
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                  {(() => {
                    const totV = planCrucesMensual.reduce((s,c) => s + c.totales.venta, 0);
                    const totC = planCrucesMensual.reduce((s,c) => s + c.totales.compra, 0);
                    const totU = planCrucesMensual.reduce((s,c) => s + c.totales.utilidad, 0);
                    const totB = planCrucesMensual.reduce((s,c) => s + c.totales.bonif, 0);
                    const mgG = totV > 0 ? totU / totV : 0;
                    const conAlertas = planCrucesMensual.filter(c => c.alertas.length > 0).length;
                    return (
                      <>
                        <div className={`p-4 rounded-xl border ${t.cardInner}`}>
                          <div className={`text-[9px] uppercase font-black tracking-widest ${t.textMuted} mb-1`}>Venta Plan</div>
                          <div className={`text-lg font-black ${t.textPurple}`}>{fmtMXN(totV)}</div>
                        </div>
                        <div className={`p-4 rounded-xl border ${t.cardInner}`}>
                          <div className={`text-[9px] uppercase font-black tracking-widest ${t.textMuted} mb-1`}>Compras</div>
                          <div className={`text-lg font-black ${t.textYellow}`}>{fmtMXN(totC)}</div>
                        </div>
                        <div className={`p-4 rounded-xl border ${t.cardInner}`}>
                          <div className={`text-[9px] uppercase font-black tracking-widest ${t.textMuted} mb-1`}>Utilidad</div>
                          <div className={`text-lg font-black text-emerald-500`}>{fmtMXN(totU)}</div>
                        </div>
                        <div className={`p-4 rounded-xl border ${t.cardInner}`}>
                          <div className={`text-[9px] uppercase font-black tracking-widest ${t.textMuted} mb-1`}>Mg% global</div>
                          <div className={`text-lg font-black text-emerald-500`}>{fmtPct(mgG)}</div>
                        </div>
                        <div className={`p-4 rounded-xl border ${t.cardInner}`}>
                          <div className={`text-[9px] uppercase font-black tracking-widest ${t.textMuted} mb-1`}>Bonif. Apertura</div>
                          <div className={`text-lg font-black ${t.textYellow}`}>{fmtMXN(totB)}</div>
                        </div>
                        <div className={`p-4 rounded-xl border ${t.cardInner}`}>
                          <div className={`text-[9px] uppercase font-black tracking-widest ${t.textMuted} mb-1`}>Alertas</div>
                          <div className={`text-lg font-black ${conAlertas > 0 ? 'text-red-500' : t.textMuted}`}>{conAlertas}</div>
                        </div>
                      </>
                    );
                  })()}
                ...
                <div className={`p-4 rounded-xl border ${t.cardInner} flex items-center justify-between flex-wrap gap-3`}>
                  <div className="flex items-center gap-3">
                    <span className={`text-[10px] font-black uppercase tracking-widest ${t.textMuted}`}>% Bonif. Apertura:</span>
                    <div className="flex items-center gap-1">
                      <NumberInputDeferred
                        value={(pctBonifApertura * 100).toFixed(2)}
                        onCommit={(parsed) => setPctBonifApertura((parsed ?? 1.2) / 100)}
                        className={`w-20 text-right font-mono text-xs px-2 py-1 rounded border ${t.input}`} />
                      <span className={`text-xs ${t.textMuted}`}>% sobre compra apertura (PVP costo)</span>
                    </div>
                  </div>
                  <p className={`text-[10px] ${t.textMuted}`}>Se aplica 1 mes antes de apertura</p>
                </div>

                {/* Selector */}
                <div className={`p-4 rounded-xl border ${t.cardInner} flex items-center justify-between flex-wrap gap-3`}>
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] font-black uppercase tracking-widest ${t.textMuted}`}>Centro:</span>
                    <select value={t5Centro} onChange={e => setT5Centro(e.target.value)}
                      className={`text-xs px-3 py-2 rounded border ${t.input} min-w-[200px]`}>
                      <option value="">— Todos (resumen) —</option>
                      {planPorCentro.map(c => (
                        <option key={c.centro} value={c.centro}>
                          {c.centro} · {c.nombre} {(centros[c.centro]?.tipo === 'APERTURA' || centros[c.centro]?.tipo === 'NUEVA') ? '🆕' : ''}
                        </option>
                      ))}
                    </select>
                  </div>
                  <div className={`flex rounded-lg border ${t.border} overflow-hidden`}>
                    <button onClick={() => setT5Vista('porGoa')}
                      className={`px-3 py-1.5 text-xs font-bold ${t5Vista === 'porGoa' ? t.btnPurple : t.btnGhost}`}>Por GOA</button>
                    <button onClick={() => setT5Vista('consolidado')}
                      className={`px-3 py-1.5 text-xs font-bold ${t5Vista === 'consolidado' ? t.btnPurple : t.btnGhost}`}>Consolidado</button>
                  </div>
                </div>

                {/* Vista todos los centros */}
                {!t5Centro && (
                  <div className={`p-4 rounded-xl border ${t.cardInner}`}>
                    <h3 className={`text-xs font-black uppercase tracking-widest mb-3 ${t.textMuted}`}>Resumen por Centro · Click para detalle</h3>
                      ...
                      <table className="w-full text-left min-w-max text-xs">
                        <thead>
                          <tr className={`text-[9px] uppercase font-black tracking-widest sticky top-0 ${isDark ? 'bg-zinc-900 text-gray-400 border-b border-zinc-800' : 'bg-gray-50 text-gray-500 border-b border-gray-200'}`}>
                            ...
                            <th className="p-2 text-right">MSI</th>
                        ...
                        <tbody className={`divide-y ${isDark ? 'divide-zinc-800/50' : 'divide-gray-100'}`}>
                          {planPorCentro.map(c => {
                            const tipo = centros[c.centro]?.tipo || 'NORMAL';
                            const totAlertas = c.cruces.reduce((s,x) => s + x.alertas.length, 0);
                            return (
                              <tr key={c.centro} className={`cursor-pointer ${isDark ? 'hover:bg-violet-900/20' : 'hover:bg-violet-50'}`}
                                  onClick={() => setT5Centro(c.centro)}>
                                <td className={`p-2 font-mono ${t.textMain}`}>{c.centro} <span className={`text-[9px] ${t.textMuted}`}>· {c.nombre}</span></td>
                                <td className="p-2">
                                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${
                                    tipo === 'APERTURA' || tipo === 'NUEVA' ? t.badgeYellow : t.badgeGray
                                  }`}>{tipo}</span>
                                </td>
                                <td className={`p-2 text-right font-mono ${t.textPurple}`}>{fmt(c.totales.venta, 0)}</td>
                                <td className={`p-2 text-right font-mono ${t.textMuted}`}>{fmt(c.totales.mkd, 0)}</td>
                                <td className={`p-2 text-right font-mono ${t.textMuted}`}>{fmt(c.totales.msi, 0)}</td>
                                <td className={`p-2 text-right font-mono ${t.textYellow}`}>{fmt(c.totales.compra, 0)}</td>
                                <td className={`p-2 text-right font-mono ${c.totales.bonif > 0 ? t.textYellow : 'opacity-30'}`}>
                                  {c.totales.bonif > 0 ? fmt(c.totales.bonif, 0) : '—'}
                                </td>
                                <td className={`p-2 text-right font-mono font-bold text-emerald-500`}>{fmt(c.totales.utilidad, 0)}</td>
                                <td className={`p-2 text-right font-mono ${c.mgRealTotal >= 0.35 ? 'text-emerald-500' : c.mgRealTotal >= 0.20 ? t.textYellow : 'text-red-500'}`}>
                                  {fmtPct(c.mgRealTotal)}
                                </td>
                                <td className="p-2 text-center">
                                  {totAlertas > 0 && (
                  ...
                  <>
                    <div className={`p-4 rounded-xl border ${t.cardInner}`}>
                      <div className="flex items-center justify-between flex-wrap gap-2">
                        <div>
                          <span className={`text-[10px] font-black uppercase tracking-widest ${t.textMuted}`}>Centro</span>
                          <h3 className={`text-lg font-black ${t.textMain}`}>
                            {t5Centro} · {centros[t5Centro]?.nombre}
                            {(centros[t5Centro]?.tipo === 'APERTURA' || centros[t5Centro]?.tipo === 'NUEVA') && (
                              <span className={`ml-2 text-xs font-bold px-2 py-0.5 rounded-full ${t.badgeYellow}`}>
                                🆕 {centros[t5Centro]?.tipo} · Abre {MESES[(centros[t5Centro]?.mesApertura || 1) - 1]}
                              </span>
                            )}
                          </h3>
                        </div>
                    ...
                    {/* Consolidado */}
                    {t5Vista === 'consolidado' && (() => {
                      const data = planPorCentro.find(c => c.centro === t5Centro);
                      if (!data) return null;
                      return (
                        <div className={`p-4 rounded-xl border ${t.cardInner}`}>
                          <h3 className={`text-xs font-black uppercase tracking-widest mb-3 ${t.textMuted}`}>Plan Mensual Consolidado</h3>
                          <div className="overflow-x-auto custom-scrollbar">
                            <table className="w-full text-left min-w-max text-xs">
                              <thead>
                                <tr className={`text-[9px] uppercase font-black tracking-widest ${isDark ? 'bg-zinc-900 text-gray-400 border-b border-zinc-800' : 'bg-gray-50 text-gray-500 border-b border-gray-200'}`}>
                              ...
                              <tbody className={`divide-y ${isDark ? 'divide-zinc-800/50' : 'divide-gray-100'}`}>
                                {[
                                  ...
                                  { k: 'msi',        label: 'Costo MSI $',   color: t.textMuted },
                                  { k: 'compra',     label: 'Compras $',     color: t.textYellow, bold: true },
                                  { k: 'bonif',      label: 'Bonif. Apertura', color: t.textYellow },
                                  ...
                                  const total = isPct
                                    ? data.mgRealTotal
                                    : data.filasMensual.reduce((s,f) => s + (f[k] || 0), 0);
                                  return (
                    ...
                    {/* Vista por GOA */}
                    {t5Vista === 'porGoa' && planCrucesMensual.filter(c => c.centro === t5Centro).map(cruce => (
                      <div key={`${cruce.centro}|${cruce.goa}`} className={`p-4 rounded-xl border ${t.cardInner}`}>
                        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                          <div className="flex items-center gap-3">
                            <span className={`text-sm font-bold px-3 py-1 rounded-full border ${t.badgePurple}`}>{cruce.goa}</span>
                            <span className={`text-[10px] ${t.textMuted}`}>
                              Mg objetivo: <strong className={t.textMain}>{fmtPct(cruce.mgObjetivo)}</strong>
                              {cruce.mgOverrideCxG && <span className={`ml-1 ${t.textYellow}`}>(override)</span>}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className={`text-[10px] ${t.textMuted}`}>Override Mg%:</span>
                            <NumberInputDeferred
                              value={cruce.mgOverrideCxG ? (cruce.mgObjetivo * 100).toFixed(1) : ''}
                              placeholder={(cruce.mgHist * 100).toFixed(1)}
                              onCommit={(parsed) => {
                                const k = `${cruce.centro}|${cruce.goa}`;
                                setMgObjetivoCentro(prev => {
                                  const n = { ...prev };
                                  if (parsed === null) delete n[k];
                                  else n[k] = parsed / 100;
                                  return n;
                        ...
                        {cruce.alertas.length > 0 && (
                          <div className="mb-3 space-y-1">
                            {cruce.alertas.map((a, i) => (
                              <div key={i} className={`p-2 rounded-lg text-[10px] font-bold ${
                                a.tipo === 'danger'
                                  ? (isDark ? 'bg-red-900/30 border border-red-500/40 text-red-300' : 'bg-red-50 border border-red-200 text-red-700')
                                  : (isDark ? 'bg-amber-900/20 border border-amber-500/40 text-amber-300' : 'bg-amber-50 border border-amber-200 text-amber-800')
                              }`}>⚠️ {a.msg}</div>
                            ))}
                          </div>
                        )}
                          ...
                          <table className="w-full text-left min-w-max text-xs">
                            ...
                            <tbody className={`divide-y ${isDark ? 'divide-zinc-800/50' : 'divide-gray-100'}`}>
                              {[
                                ...
                                { k: 'costo',      label: 'Costo (deriv)', color: t.textGray },
                                { k: 'compra',     label: 'Compras',     color: t.textYellow, bold: true, mark: true },
                                { k: 'bonif',      label: 'Bonif. Apertura', color: t.textYellow },
                                { k: 'utilidad',   label: 'Utilidad',    color: 'text-emerald-500', bold: true },
                                { k: 'mgPct',      label: 'Mg %',        color: 'text-emerald-500', isPct: true },
                                { k: 'invFinal',   label: 'Inv. Final',  color: t.textMuted },
                              ].map(({k, label, color, isPct, bold, mark}) => {
                                const total = isPct ? cruce.mgRealTotal : cruce.filasMensual.reduce((s,f) => s + (f[k] || 0), 0);
                                return (
                                  <tr key={k} className={bold ? (isDark ? 'bg-violet-900/5' : 'bg-violet-50/30') : ''}>
                                    <td className={`p-2 sticky left-0 bg-inherit ${bold ? 'font-black' : 'font-bold'} ${color}`}>{label}</td>
                                    {cruce.filasMensual.map((f, i) => (
                                      <td key={i} className={`p-2 text-right font-mono ${color} ${bold ? 'font-bold' : ''} ${!f.aplica ? 'opacity-30' : ''}`}>
                                        {isPct ? fmtPct(f[k]) : (f[k] !== 0 ? fmt(f[k], 0) : '—')}
                                        {mark && f.compraRebalanceada && (
                                          <span className="ml-0.5 text-[9px] text-amber-500" title="Compra rebalanceada (era negativa)">⚖️</span>
                                        )}
                                      </td>
                                    ))}
                                    <td className={`p-2 text-right font-mono font-black ${color}`}>
