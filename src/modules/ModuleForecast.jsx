...
// IMPORTANTE: En tu entorno local, descomenta tu import real de iconos:
import * as Icons from '../utils/icons';
// Simulador temporal para la vista previa:
...
// Se recibe isDark como prop para que obedezca al switch de la App principal
export default function ModuleForecast({ isDark = true }) {
    ...
    <div className={`min-h-screen ${themeColors.bg} flex flex-col font-sans ${themeColors.text} overflow-hidden transition-colors duration-300`}>
      <header className={`${themeColors.header} border-b ${themeColors.border} px-6 py-4 flex justify-between items-center backdrop-blur-md sticky top-0 z-30 shadow-2xl`}>
        <div className="flex items-center gap-4">
          <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className={`p-2 hover:${isDark ? 'bg-zinc-800' : 'bg-slate-100'} rounded-lg transition-all`}><Icons.Menu size={20}/></button>
          <div className="flex items-center gap-3 text-left">
            <div className="bg-violet-600 p-2 rounded-xl text-white shadow-lg"><Icons.Layers size={20}/></div>
        ...
        <div className="flex gap-3 items-center">
            {/* El botón de cambio de tema fue removido de aquí para que use el global de la App */}
            <button onClick={() => {
            ...
            }} className="flex items-center gap-2 bg-violet-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-violet-500 shadow-xl transition-all"><Icons.Plus size={18}/> Nueva Marca</button>
        </div>
      </header>
        ...
        <aside className={`sidebar-transition h-full ${themeColors.sidebar} border-r ${themeColors.border} flex flex-col ${isSidebarOpen ? 'w-80' : 'w-0 overflow-hidden opacity-0'} transition-all duration-300`}>
            ...
            {brands.map(brand => (
              <div key={brand.id} onClick={() => setSelectedBrandId(brand.id)} className={`w-full group p-4 rounded-2xl transition-all flex items-center justify-between cursor-pointer border ${selectedBrandId === brand.id ? (isDark ? 'bg-zinc-900 border-violet-500/50 shadow-inner' : 'bg-violet-50 border-violet-200 shadow-sm') : 'bg-transparent border-transparent hover:bg-zinc-900/50'}`}>
                ...
                <div className={`flex gap-1 ${selectedBrandId === brand.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}>
                  <button onClick={(e) => { e.stopPropagation(); setIsEditing(brand.id); setEditForm({ name: brand.name, data: brand.data.join(' '), unit: brand.unit }); }} className="p-2 hover:bg-zinc-800 rounded-lg text-zinc-500 hover:text-violet-400"><Icons.Edit3 size={14}/></button>
                  <button onClick={(e) => { e.stopPropagation(); setBrands(brands.filter(b => b.id !== brand.id)); if(selectedBrandId === brand.id) setSelectedBrandId(null); }} className="p-2 hover:bg-zinc-800 rounded-lg text-zinc-500 hover:text-rose-500"><Icons.Trash2 size={14}/></button>
                </div>
              </div>
            ))}
          </div>
          <div className={`p-4 border-t ${themeColors.border} space-y-2 ${isDark ? 'bg-zinc-950/80' : 'bg-slate-50'}`}>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => {const b=new Blob([JSON.stringify({brands})],{type:'application/json'});const u=URL.createObjectURL(b);const l=document.createElement('a');l.href=u;l.download='goplanner_sesion.json';l.click();}} className={`flex items-center justify-center gap-2 ${isDark ? 'bg-zinc-900' : 'bg-white'} hover:${isDark ? 'bg-zinc-800' : 'bg-slate-100'} p-3 rounded-xl text-[10px] font-bold border ${themeColors.border} transition-all active:scale-95`}><Icons.Save size={14}/> RESPALDAR</button>
              <label className={`flex items-center justify-center gap-2 ${isDark ? 'bg-zinc-900' : 'bg-white'} hover:${isDark ? 'bg-zinc-800' : 'bg-slate-100'} p-3 rounded-xl text-[10px] font-bold border ${themeColors.border} cursor-pointer transition-all active:scale-95`}><Icons.Upload size={14}/> CARGAR<input type="file" className="hidden" accept=".json" onChange={(e)=>{const f=e.target.files[0]; if(!f) return; const r=new FileReader(); r.onload=(ev)=>{try { const d=JSON.parse(ev.target.result); if(d.brands){ setBrands(d.brands); if(d.brands.length > 0) setSelectedBrandId(d.brands[0].id); } } catch(err){ alert("Error de carga"); }}; r.readAsText(f); e.target.value="";}} /></label>
            </div>
          </div>
        </aside>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 text-left">
                  <div className="flex-1"><h2 className={`text-6xl font-black ${themeColors.heading} tracking-tighter uppercase leading-none`}>{currentBrand.name}</h2>
                    <div className="flex gap-4 mt-4">
                      <span className={`${themeColors.card} px-4 py-2 rounded-full border ${themeColors.border} text-[10px] font-black uppercase tracking-widest text-zinc-500 shadow-sm flex items-center gap-2`}><Icons.Calendar size={14} className="text-violet-500"/> Plan por {currentBrand.unit}</span>
                  ...
                  <div className={`${themeColors.card} border ${themeColors.border} px-8 py-6 rounded-[32px] flex items-center gap-5 shadow-2xl transition-all hover:border-yellow-500/30`}><div className="bg-yellow-500 p-3 rounded-2xl text-black shadow-lg shadow-yellow-500/20 animate-pulse"><Icons.Trophy size={24}/></div><div><p className="text-[10px] font-black text-zinc-500 uppercase mb-0.5 tracking-widest leading-none text-left">Mejor Ajuste</p><p className={`font-black text-2xl ${themeColors.heading} tracking-tight uppercase`}>{winner.name}</p></div></div>
                </div>
