  ...
  return (
    <div className={`min-h-screen ${colors.bg} flex flex-col font-sans transition-colors duration-500`}>
      <header className={`${colors.header} border-b ${colors.border} px-6 py-4 flex justify-between items-center backdrop-blur-md sticky top-0 z-30 shadow-sm`}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col text-left">
            <h1 className={`text-xl font-black ${colors.textHeading} tracking-tighter`}>GO <span className="text-violet-500">PLANNER</span></h1>
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className={`w-72 border-r ${colors.border} ${isDark ? 'bg-zinc-950' : 'bg-white'} ${!isSidebarOpen && 'hidden'} transition-all duration-300`}>
          ...
          <div className="p-2 space-y-1 overflow-y-auto">
            {brands.map(b => (
              <button key={b.id} onClick={() => setSelectedBrandId(b.id)} className={`w-full text-left p-4 rounded-2xl transition-all flex justify-between items-center ${selectedBrandId === b.id ? 'bg-violet-600 text-white shadow-lg' : 'text-zinc-500 hover:bg-zinc-800/50'}`}>
                <span className="font-bold truncate pr-2 uppercase text-xs">{b.name}</span>
          ...
          {!currentBrand && !isEditing && (
            <div className="flex items-center justify-center min-h-[70vh]">
                <div className={`max-w-2xl ${colors.card} border-4 border-violet-600 p-12 rounded-[48px] shadow-[0_0_50px_-12px_rgba(124,58,237,0.3)] text-center animate-fade-in`}>
                    <div className="bg-violet-600 w-16 h-16 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-lg">
                    ...
                    <div className="grid grid-cols-2 gap-6 text-left">
                        <div className="bg-zinc-950/50 border border-violet-500/20 p-6 rounded-3xl">
                            <h4 className="text-violet-500 font-black text-xs uppercase mb-3 flex items-center gap-2"><FileText size={14}/> Formato CSV</h4>
                            <p className="text-[10px] text-zinc-500">Columna A: Marca.<br/>Columnas B+: Ventas.<br/><code className="block mt-2 font-mono text-violet-400">Marca, 1200, 1000...</code></p>
                        </div>
                        <div className="bg-zinc-950/50 border border-yellow-500/20 p-6 rounded-3xl">
                            <h4 className="text-yellow-500 font-black text-xs uppercase mb-3 flex items-center gap-2"><Zap size={14}/> Pegado Excel</h4>
                            <p className="text-[10px] text-zinc-500">Omitimos comas de miles automáticamente.<br/><span className="font-bold">"1,000"</span> → 1000.</p>
                        </div>
                    </div>
                </div>
            </div>
             ...
             <div className={`${colors.card} p-10 rounded-[40px] mb-8 border border-zinc-700 shadow-2xl animate-fade-in`}>
                ...
                <div className="grid grid-cols-2 gap-8 text-left">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                            {['Meses', 'Semanas'].map(u => <button key={u} onClick={() => setEditForm({...editForm, unit: u})} className={`flex-1 p-4 rounded-2xl font-bold text-xs border ${editForm.unit === u ? 'bg-violet-600 text-white border-violet-500 shadow-lg' : 'bg-black text-zinc-600 border-zinc-800 hover:border-zinc-700'}`}>{u}</button>)}
                        </div>
                    </div>
                </div>
                <div className="mt-8 space-y-2 text-left">
                    <label className="text-[10px] font-black uppercase text-yellow-500 tracking-widest">Histórico (Pega de Excel)</label>
                    <textarea className="w-full bg-black border border-zinc-800 p-6 rounded-3xl text-white h-44 font-mono text-xs outline-none focus:border-yellow-500" value={editForm.data} onChange={e => setEditForm({...editForm, data: e.target.value})} />
                </div>
            ...
            <div className="space-y-8 animate-fade-in text-left">
              <div className="flex justify-between items-end">
                ...
                <div className={`${colors.card} border ${colors.border} p-6 rounded-[32px] flex items-center gap-5 shadow-2xl`}>
                    ...
                    <div><p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1 leading-none">Best Performer</p><p className={`font-black text-xl ${colors.textHeading} tracking-tight uppercase leading-none`}>{winner.name}</p></div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-6">
                {[
                    ...
                    <div key={i} className={`${m.bg} p-6 rounded-[32px] border ${colors.border} shadow-lg text-center`}>
              ...
              <div className={`${isDark ? 'bg-zinc-950' : 'bg-white'} p-8 rounded-[48px] border ${colors.border} h-[520px] shadow-2xl relative overflow-hidden`}>
                <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-6 px-4">
                    <div className="flex gap-6">
                        <span className="flex items-center gap-2 text-[10px]"><div className="w-3 h-3 rounded-full bg-zinc-700"></div> Real</span>
                        <span className="flex items-center gap-2 text-[10px]"><div className="w-3 h-3 rounded-full bg-violet-600"></div> Modelo</span>
                        <span className="flex items-center gap-2 text-[10px]"><div className="w-3 h-3 rounded-full bg-yellow-500"></div> Forecast</span>
                    </div>
                    <span className="text-yellow-500/40">--- Acumulado Fcst (Eje Derecho)</span>
                </div>
              ...
              <div className={`${colors.card} border ${colors.border} p-8 rounded-[40px] flex justify-between items-center shadow-2xl`}>
                    ...
                    <div className="flex gap-4">
                        ...
                        <button onClick={() => setAssortmentStatus("Data Vinculada!")} className="bg-violet-600 text-white px-8 py-4 rounded-2xl font-black text-xs shadow-xl shadow-violet-900/20 flex items-center gap-2 hover:scale-105 transition-all uppercase"><ShoppingCart size={20}/> {assortmentStatus} <span className="bg-white/20 px-1 rounded text-[8px] font-black ml-1">BETA</span></button>
                    </div>
              </div>
              ...
              <div className={`${isDark ? 'bg-zinc-950' : 'bg-white'} rounded-[48px] p-10 border ${colors.border} shadow-inner`}>
                <div className="flex items-center justify-between mb-10 text-left">
                    ...
                    <button onClick={runOptimizer} className="bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 px-6 py-2.5 rounded-xl text-[10px] font-black transition-all hover:bg-yellow-500 hover:text-black uppercase flex items-center gap-2 shadow-sm shadow-yellow-500/10"><Zap size={14}/> Optimizar Mejor Ajuste</button>
                </div>
